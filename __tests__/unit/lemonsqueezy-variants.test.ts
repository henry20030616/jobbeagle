...
describe('Lemon Squeezy variant resolution', () => {
  ...
  it('single_lite falls back to BASIC_OVERAGE when SINGLE_LITE unset', () => {
  ...
  it('prefers SINGLE_LITE when set', () => {
    ...
    expect(resolveLemonSqueezyVariant('single_lite')).toBe('1111');
  });
  ...
  it('reports missing variants', () => {
    ...
    expect(missing.some((m) => m.includes('SINGLE_FULL'))).toBe(true);
  });
});
