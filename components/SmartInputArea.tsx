    ...
    <div className={compact ? 'flex flex-col flex-1 h-full min-h-0 gap-2' : 'space-y-0'}>
      <div className={`relative ${compact ? 'flex flex-col flex-1 min-h-0 h-full' : ''}`}>
        ...
        <textarea
          ...
          className={`w-full ${compact ? 'flex-1 basis-0 min-h-[min(48vh,26rem)]' : 'min-h-[220px]'} bg-slate-900/30 border-2 border-dashed rounded-xl ${compact ? 'p-4 text-lg' : 'p-5 text-base'} text-slate-200 placeholder-slate-500 focus:ring-2 focus:border-solid transition-all ${compact ? 'resize-none' : 'resize-y'} disabled:opacity-60 ${borderClass}`}
