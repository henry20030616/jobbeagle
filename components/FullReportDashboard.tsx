...
import { AlertTriangle, Users, MessageSquare, HandCoins, Globe } from 'lucide-react';
...
export default function FullReportDashboard({ report, embedded = false }: FullReportDashboardProps) {
  return (
    <div className={`rounded-2xl border border-violet-500/30 bg-slate-900/80 overflow-hidden ${embedded ? '' : 'shadow-xl'}`}>
      ...
      <div className="p-6 space-y-6">
        {report.online_intel_warning ? (
          <div className="rounded-xl bg-red-600/20 border border-red-500/40 p-4 flex gap-3">
            ...
            <div>
              <p className="text-xs text-red-300 uppercase tracking-wider mb-1 font-bold">Live Intel Warning</p>
              <p className="text-sm text-red-100/90 leading-relaxed">{report.online_intel_warning}</p>
            </div>
          </div>
        ) : (
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-5">
          ...
          <ol className="space-y-3">
            {report.custom_star_interview_bank.map((q, i) => (
              ...
              >
                <span className="text-violet-400 font-mono text-xs font-bold mr-2">Q{i + 1}</span>
                {q}
              </li>
            ))}
          </ol>
