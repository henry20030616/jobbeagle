...
import { generateRecruitmentVideo } from '../services/geminiService';
import { 
...
const VideoCard: React.FC<VideoCardProps> = ({ job, isActive }) => {
  ...
  const handleGenerateRelevantVideo = async (e: React.MouseEvent) => {
    e.stopPropagation();
    ...
    try {
        const prompt = `A cinematic, professional vertical 9:16 video showing a ${job.jobTitle} workspace at ${job.companyName}. Photorealistic, high quality.`;
        const uri = await generateRecruitmentVideo(prompt);
        setVideoUrl(`${uri}&key=${process.env.API_KEY}`);
    } catch (err) {
        console.error(err);
        alert("AI 影片生成失敗，請確認 API Key。");
    } finally {
    ...
    <div className="relative w-full h-full bg-black overflow-hidden border-b border-gray-800 select-none">
      ...
      >
        {isGenerating && (
            <div className="absolute inset-0 z-50 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm">
                ...
                <p className="text-white font-bold">正在生成與職缺相關的 AI 影片...</p>
            </div>
        )}
      ...
      <div className="absolute right-2 bottom-24 flex flex-col items-center gap-5 z-20 pb-4">
        ...
        <div className="flex flex-col items-center gap-1">
            <button 
                onClick={handleGenerateRelevantVideo}
                className={`p-2.5 rounded-full bg-gradient-to-tr from-cyan-500 to-purple-600 shadow-lg text-white transition-all active:scale-90 ${isGenerating ? 'animate-pulse' : 'hover:scale-110'}`}
                title="AI 生成相關影片"
            >
              <Sparkles size={26} />
            </button>
            <span className="text-[9px] font-bold drop-shadow-md text-cyan-300">AI 相關</span>
        </div>
      ...
      {/* --- Full Details Modal --- */}
      {showFullDetails && (
        <div className="absolute inset-x-0 bottom-0 z-40 bg-slate-900/95 backdrop-blur-xl rounded-t-2xl border-t border-white/10 p-5 pb-24 animate-in slide-in-from-bottom-full duration-300">
      ...
      {showApplyModal && (
          <div className="absolute inset-x-0 bottom-0 z-50 bg-slate-900 rounded-t-3xl p-6 pb-24 border-t border-cyan-500/30 animate-in slide-in-from-bottom-full duration-300 shadow-2xl">
              ...
              {applyState === 'idle' && (
                  <form onSubmit={handleApplySubmit} className="space-y-5 animate-in fade-in duration-300">
