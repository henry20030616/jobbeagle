...
export const generateJobScriptAndImage = async (
  ...
  // 1. Generate Script and Visual Description using 全域配置的 Gemini 模型
  const modelId = GEMINI_VIDEO_MODEL;
  ...
  try {
    ...
    // 已停用：僅使用 Gemini 3.1 Flash-Lite，不支援圖片生成
    let thumbnailBase64 = undefined;

    // 圖片生成功能已停用，僅使用 Gemini 3.1 Flash-Lite
    // try {
    //     const imagePrompt = `
...
export const generateRecruitmentVideo = async (
    ...
    // 已停用：僅使用 Gemini 3.1 Flash-Lite，不支援視頻生成
    // 此功能使用 veo-3.1-fast-generate-preview，不符合僅使用 Gemini 3.1 Flash-Lite 的要求
    throw new Error("Video generation is disabled. Only Gemini 3.1 Flash-Lite is allowed.");
