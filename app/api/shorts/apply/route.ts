...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send email via Resend → directly to company's contact_email
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      html: `
        <div style="font-family:-apple-system,sans-serif;max-width:580px;margin:0 auto;padding:24px;">
          ...
          ${resumeUrl ? `
          <div style="background:#f0f9ff;border-radius:8px;padding:16px;margin-bottom:16px;border:1px solid #bae6fd;">
            <p style="margin:0 0 10px 0;font-size:11px;color:#0369a1;text-transform:uppercase;letter-spacing:.05em;">履歷</p>
            <p style="margin:0 0 12px 0;color:#64748b;font-size:13px;">${resumeFileName || '履歷檔案'}</p>
            <a href="${resumeUrl}" style="background:#0ea5e9;color:white;padding:9px 20px;border-radius:6px;text-decoration:none;font-size:13px;font-weight:600;">📄 下載履歷</a>
          </div>` : '<p style="color:#94a3b8;font-size:13px;margin-bottom:16px;">（未附上履歷）</p>'}
